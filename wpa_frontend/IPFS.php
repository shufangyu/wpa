...
class IPFS
{
    ...
    public function get($hash)
    {
        ...
        return $this->curl($this->getApiUrl() . "/get/$hash");
        //return $this->curl($this->getApiUrl() . "/get?arg=$hash&output='" . public_path('storage'). "'");
    }
    ...
    private function setCurlGet($url) {
        curl_setopt($this->curl, CURLOPT_POST, 1);
        //echo $url;
        //echo "<br>";
        $fileNmae = substr($url, -46) . '.gz';
        $fileSavePath = public_path('storage').'/ipfsGet/'. $fileNmae;
        $outfile = fopen($fileSavePath, 'w'); //儲存到本地檔案的檔名 
